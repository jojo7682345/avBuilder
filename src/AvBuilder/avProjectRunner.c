...
struct Value compareNumberString(Project* project, struct Value left, enum ComparisonOperator operator, struct Value right){
	// TODO: implement number string comparison
...
struct Value performComparison(struct ComparisonExpression_S expression, Project* project){
	...
	if(left.type == VALUE_TYPE_ARRAY){
		switch(right.type){
			case VALUE_TYPE_ARRAY:
				return compareArrays(project, left, expression.operator, right);
			case VALUE_TYPE_NUMBER:
				return compareArrayNumber(project, left, expression.operator, right);
			case VALUE_TYPE_STRING:
				return compareArrayString(project, left, expression.operator, right);
			default: break;
		}
	}
	if(left.type == VALUE_TYPE_NUMBER){
		switch(right.type){
			case VALUE_TYPE_ARRAY:
				return compareNumberArray(project, left, expression.operator, right);
			case VALUE_TYPE_NUMBER:
				return compareNumbers(project, left, expression.operator, right);
			case VALUE_TYPE_STRING:
				return compareNumberString(project, left, expression.operator, right);
			default: break;
		}
	}
	if(left.type == VALUE_TYPE_STRING){
		switch(right.type){
			case VALUE_TYPE_ARRAY:
				return compareStringArray(project, left, expression.operator, right);
			case VALUE_TYPE_NUMBER:
				return compareStringNumber(project, left, expression.operator, right);
			case VALUE_TYPE_STRING:
				return compareStrings(project, left, expression.operator, right);
			default: break;
		}
	}
...
Project* importProject(AvString projectFile, bool32 local, Project* baseProject){
	...
	projectCreate(project, projectFileName, projectFile, projectFileContent, local);
...
struct VariableDescription importVariable(struct ImportDescription import, Project* project){
	...
	for(uint32 index = 0; index < avDynamicArrayGetSize(project->importedProjects); index++) { 
		...
		{ 
			if(avStringEquals(import.importFile, element->projectFileName) && import.isLocalFile == element->isLocal){
				found = true;
				extProject = element;
				break;
			}
